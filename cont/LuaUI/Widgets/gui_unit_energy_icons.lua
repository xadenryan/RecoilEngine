...
--------------------------------------------------------------------------------
--
--  file:    gui_unit_energy_icons.lua
...
if isMacOS then
	return loader.BuildMacOSFallbackWidget({
		...
		desc = "Disable SSBO-backed unit energy icons on macOS",
		warning = "[BAR widget proxy] disabling Unit Energy Icons on macOS because Apple OpenGL 4.1 does not support the required SSBO shader path",
	})
end

return loader.LoadPatchedModWidget(
	getfenv(1),
	LUAUI_DIRNAME .. "Widgets/gui_unit_energy_icons.lua",
	nil,
	"load original Unit Energy Icons widget"
)
